...
const { parseCsvEmailFile, getEmailList } = require('../../api/service/send-email-service');
...
describe('Send Email Service - Parsing Email CSV File and Build Email String', function() {
  ...
  it('correctly ignores first few rows that contains only "metadata"', function() {
    ...
    expect(actualEmailList[0].englishName).to.eq(expectedEmailListItem1.englishName);
    expect(actualEmailList[0].chineseName).to.eq(expectedEmailListItem1.chineseName);

    const lastItemIndex = actualEmailList.length - 1;
    expect(actualEmailList[lastItemIndex].email).to.eq(expectedLastEmailListItem.email);
    expect(actualEmailList[lastItemIndex].englishName).to.eq(expectedLastEmailListItem.englishName);
    expect(actualEmailList[lastItemIndex].chineseName).to.eq(expectedLastEmailListItem.chineseName);
  });
  ...
  it('validates metadata rows correctly (The metadata row uses "email" column to describe the row item)', function() {
    ...
    const isMetadata = emails.map((email) => EmailListItem.isCsvMetaData(email.email, email.englishName, email.chineseName));
  ...
  it('validate email format correctly', function() {
    ...
    const actualvalidation = emails.map((email) => EmailListItem.isValidEmail(email.email));
